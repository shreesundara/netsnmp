...
"""
        ...
        self.globalVrfSupportedSchemaTuple = ()
        self.vrfSupportedSchemasUsedByMibBeingProcessed = []
        self.snmp_get_tbl_context_fns = []
        self.tableRows = {}
        self.enumSymbols = {}
...
### Subparts generation functions
    ...
    def genCode(self, ast, symbolTable, **kwargs):
        ...
        self.loadVrfSupportedSchemaDetailsFromConf()

        self.checkIfMibUsesVrfSupportedSchema(self.moduleName[0].replace('-','_'))
    ...
    def loadVrfSupportedSchemaDetailsFromConf(self):
        self.globalVrfSupportedSchemaTuple = ('ospf_router')
    ...
    def genPublisherFiles(self, moduleName):
        ...
        for table in set(self.publisherTables):
            ...
            publisherFileString += 'bool '+table+'_publish_request(struct ovsdb_idl *idl,const char* containerName) {\n\n'
        ...
        for table in set(self.publisherTables):
            publisherHeaderFileString += 'bool '+table+'_publish_request(struct ovsdb_idl *idl,const char* containerName);\n\n'
...
"""
    ...
    def checkIfMibUsesVrfSupportedSchema(self,moduleName):
        #scan through all scalars and tables of this mib from mapping file and get the list of used vrf-table-column
        #for mib-scalars will check only the ovsTable col of mapping fole.
        #for mib-tables check with RootOvsTable and ovsTable of each idx or column.

        for sym in self.scalarSymbols:
            name = self._out[sym]['name']
            if name not in self.jsonData:
                continue
            if self.jsonData[name]['OvsTable'] and self.jsonData[name]['OvsColumn']:
                #Assuming the entire schema table participates in vrf
                if self.jsonData[name]['OvsTable'] in self.globalVrfSupportedSchemaTuple:
                    if self.jsonData[name]['OvsTable'] not in self.vrfSupportedSchemasUsedByMibBeingProcessed:
                        self.vrfSupportedSchemasUsedByMibBeingProcessed.append(self.jsonData[name]['OvsTable'])
                        #return

        for key in self.tables.keys():
            tableName = key
            if tableName not in self.jsonData:
                continue
            ovsdbTables = self.getOvsdbRowsForTable(tableName)
            for individTable in ovsdbTables:
                if individTable in self.globalVrfSupportedSchemaTuple:
                    if individTable not in self.vrfSupportedSchemasUsedByMibBeingProcessed:
                        self.vrfSupportedSchemasUsedByMibBeingProcessed.append(individTable)
                        #return

        #self.vrfSupportedSchemasUsedByMibBeingProcessed = []
        return

    def supportsVRFContext(self,table = None):
        if not table:
            if len(self.vrfSupportedSchemasUsedByMibBeingProcessed) > 0:
                return True
        if isinstance(table,list):
            for tbl in table:
                if len(self.vrfSupportedSchemasUsedByMibBeingProcessed) > 0 and tbl in self.vrfSupportedSchemasUsedByMibBeingProcessed:
                    return True
        else:
            if len(self.vrfSupportedSchemasUsedByMibBeingProcessed) > 0 and table in self.vrfSupportedSchemasUsedByMibBeingProcessed:
                return True
        return False
    ...
    def genCFile(self, moduleName, data):
        ...
        if self.supportsVRFContext():
            scalarFileString += 'void ' + moduleName.lower() + '_scalars_ovsdb_idl_init() {\n'
            scalarFileString += """ovsdb_idl_add_table(idl, &ovsrec_table_vrf);
ovsdb_idl_add_column(idl, &ovsrec_vrf_col_name);
"""
            for vrfSchemaUsed in self.vrfSupportedSchemasUsedByMibBeingProcessed:
                scalarFileString += 'ovsdb_idl_add_column(idl, &ovsrec_vrf_col_' + vrfSchemaUsed + 's);\n' #i.e. ospf_routers

            uniqueOvsdbIdlClms = set()
            for sym in self.scalarSymbols:
                name = self._out[sym]['name']
                if name not in self.jsonData:
                    continue
                if self.jsonData[name]['OvsTable'] and self.jsonData[name]['OvsColumn'] and self.jsonData[name]['OvsTable']+':'+self.jsonData[name]['OvsColumn'] not in uniqueOvsdbIdlClms:
                    scalarFileString += 'ovsdb_idl_add_column(idl, &ovsrec_' + self.jsonData[name]['OvsTable'] + '_col_' + self.jsonData[name]['OvsColumn'] + ');\n'
                    uniqueOvsdbIdlClms.add(self.jsonData[name]['OvsTable']+':'+self.jsonData[name]['OvsColumn'])
            scalarFileString += '}\n\n'
        ...
        for sym in self.scalarSymbols:
            ...
            if tble and self.supportsVRFContext(tble):
                outStr += 'void init_' + name + '(char* snmp_ctx) {\n'
            else:
                outStr += 'void init_' + name + '(void) {\n'
            outStr += 'const oid ' + name + '_oid[] = ' + str(oidStr).replace('[', '{').replace(']','}') + ';\n'
            outStr += 'netsnmp_handler_registration *reginfo;\n'
            outStr += 'reginfo = netsnmp_create_handler_registration("' + name + '", handler_' + name + ',' + name + '_oid, OID_LENGTH(' + name + '_oid), HANDLER_CAN_RWRITE);\n'

            if tble and self.supportsVRFContext(tble):
                outStr += """/*
* context registration with scalar.
* it's needed while retrieve the scalar via context for filtering context
*/
if(snmp_ctx !=  NULL)
{
...
"""
            outStr += 'netsnmp_register_scalar(reginfo);\n\n'
            if not (tble and self.supportsVRFContext(tble)):
                if jsonValue['OvsTable'] and jsonValue['OvsColumn']:
            ...
            if not jsonValue['OvsTable']:
                ...
                if tble and self.supportsVRFContext(tble):
                    outStr += 'const struct ovsrec_' + jsonValue['OvsTable'] + ' *' + jsonValue['OvsTable'] + '_row = snmp_get_' + jsonValue['OvsTable'] + '_row_from_context(reginfo->contextName);\n'
                    if jsonValue['OvsTable'] not in self.snmp_get_tbl_context_fns:
                        self.snmp_get_tbl_context_fns.append(jsonValue['OvsTable'])
                        self.customFileHeaderString += 'const struct ovsrec_' + jsonValue['OvsTable'] + ' * snmp_get_' + jsonValue['OvsTable'] + '_row_from_context(const char* context_name);\n'
                else:
                ...
                if jsonValue.has_key('isPublisher') and jsonValue['isPublisher']:
                    if isinstance(jsonValue['isPublisher'],list) :
                        ...
                        for table in pubTbl:
                            outStr += 'if(!' + table + '_publish_request(idl,reginfo->contextName)) {\n return -1;\n}\n\n'
                        ...
                        outStr += 'if(!' + jsonValue['isPublisher'] + '_publish_request(idl,reginfo->contextName)) {\n return -1;\n}\n\n'
...
"""
        ...
        pluginsFileString += '#include "' + moduleName + '_custom.h"\n'
        ...
        if self.supportsVRFContext():
            pluginsFileString += moduleName.lower() + '_scalars_ovsdb_idl_init();\n'
        for codeSym in self.scalarSymbols:
            name = codeSym
            if name not in self.jsonData:
                continue
            tble = self.jsonData[name]['OvsTable'];
            if tble and self.supportsVRFContext(tble ):
                pluginsFileString += 'init_' + name + '(NULL);\n'
            else :
        ...
        if self.supportsVRFContext():
            pluginsFileString +=  'snmp_context_initialize_' + moduleName.lower().replace('_mib','') + '();\n'
            self.customFileHeaderString += 'void snmp_context_initialize_' + moduleName.lower().replace('_mib','') + '(void);\n'
        pluginsFileString += '}\n'
    ...
    def getFirstIntanceStringForTable(self, tableName, tables):
        outStr = ''
        for tbl in tables:
            #snmp_get_ospf_router_from_context
            if tbl and self.supportsVRFContext(tbl):
            #snmp_get_ospf_router_from_context
                outStr += tbl + '_row = snmp_get_' + tbl + '_row_from_context(container->container_name);\n'
                if tbl not in self.snmp_get_tbl_context_fns:
                    self.snmp_get_tbl_context_fns.append(tbl)
                    self.customFileHeaderString += 'const struct ovsrec_' + tbl + ' * snmp_get_' + tbl + '_row_from_context(const char* context_name);\n'
                ...
                if isinstance(idxPubTbl,list):
                    for tbl in idxPubTbl :
                        if tbl not in PublisherTablesForThisTable:
                            outStr += 'if(!' + tbl + '_publish_request(idl,container->container_name)) {\n return -1;\n}\n\n'
                            PublisherTablesForThisTable.append(tbl)
                            self.publisherTables.append(tbl)
                elif idxPubTbl not in PublisherTablesForThisTable:
                    outStr += 'if(!' + idxPubTbl + '_publish_request(idl,container->container_name)) {\n return -1;\n}\n\n'
        ...
        for col in self.tableRows[self.tables[tableName]['row']]['columns']:
            ...
            if self.jsonData[tableName]['Columns'][col['name']].has_key('isPublisher') and self.jsonData[tableName]['Columns'][col['name']]['isPublisher']:
                ...
                if isinstance(colPubTbl,list):
                    ...
                    outStr += 'if(!' + colPubTbl + '_publish_request(idl,container->container_name)) {\n return -1;\n}\n\n'
...
"""
            ...
            if self.supportsVRFContext(ovsdbTables):
                tableFileString += '_' + tableName + '_initialize_interface(user_context,flags,NULL);\n\n'
            else:
...
"""
            ...
            if self.supportsVRFContext(ovsdbTables):
                tableInterfaceString += 'void _' + tableName + '_initialize_interface(' + tableName + '_registration *reg_ptr, u_long flags, char* snmp_ctx) {\n'
            else:
            ...
            tableInterfaceString += """    /*
* context registration with mib-table.
...
if(snmp_ctx !=  NULL)
{
    reginfo->contextName = strdup(snmp_ctx);
"""
            tableInterfaceString +=  tableName + '_if_ctx.container->container_name = strdup(snmp_ctx);\n}\n\n'
            ...
            if ovsdbTables and self.supportsVRFContext(ovsdbTables):
                tableInterfaceHeaderString += 'void _' + tableName + '_initialize_interface(' + tableName + '_registration *user_ctx, u_long flags, char* snmp_ctx);\n'
            else:
...
"""
    ...
    def genHeaderFile(self, moduleName):
        ...
        if self.supportsVRFContext():
            headerString += 'void ' + moduleName.lower() + '_scalars_ovsdb_idl_init();\n'
        ...
        for codeSym in self.scalarSymbols:
            ...
            tble = self.jsonData[name]['OvsTable']
            if tble and self.supportsVRFContext(tble):
                headerString += 'void init_' + name + '(char *snmp_ctx);\n'
            else:
